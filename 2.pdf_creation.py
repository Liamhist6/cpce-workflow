...
def create_trajectory_map(metadata_path, map_path, alpha3_code):
    ...
    ax.plot(df.GPSLongitude, df.GPSLatitude, color='tab:red', linewidth=0.1)
...
def create_pdf_preview(session_path: Path):
    ...
    map_temp_path = Path(session_path, f"{session_path.name}_map.png")
    ...
    if create_trajectory_map(metadata_path, map_temp_path, alpha3_code):
        ...
        with Image.open(map_temp_path) as image_map:
    ...
    list_of_images = [img for img in sorted(list(Path(session_path, "DCIM").iterdir())) if img.suffix.lower() in [".png", ".jpg", ".jpeg"]]
...
def main():
    SESSION_FOLDER = Path("F:/", "CLEAN_DATA")
